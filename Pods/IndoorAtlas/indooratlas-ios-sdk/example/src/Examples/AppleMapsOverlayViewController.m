...
- (void)fetchImage:(IAFloorPlan*)floorPlan
{
    ...
    [NSURLConnection sendAsynchronousRequest:urlRequest queue:self.imageFetchQueue completionHandler:^(NSURLResponse *response, NSData *imageData, NSError *error) {
        if (!error) {
...
- (void)indoorLocationManager:(IALocationManager*)manager didEnterRegion:(IARegion*)region
{
    ...
    IAFloorPlan *fp = region.floorplan;
    if (fp != nil) {
        // use stored floor plan meta data
        self.floorPlan = fp;
        [self fetchImage:fp];
    } else {
        NSLog(@"There was no floorplan in region event!");
    }
}
